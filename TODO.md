...
Built on `feat/gui-mode` this session:
...
- [x] E1. Persist session rename across server restart/resume (in-memory only today; needs a {ccSessionId: name} map).
...
- [x] G1. Re-founded cockpit interaction on the Claude Agent SDK (query() driving the user's own subscription-auth claude; verified 2026-06-27). PTY/terminal substrate removed entirely - cc-cockpit is SDK-only, no PTY fallback. Reshaped A2, A4 and C1. The only remaining gap, 5h/7d usage windows from rate_limit_event, is tracked in C1.
  ...
  - [x] G1.3. Brainstorm -> write the design spec + implementation plan; decide what the GUI renders when PTY is no longer primary, and the terminal-fallback behavior.
...
- [x] H1. Floating header panels (In-session todo / Topics / TODO.MD) should match the session chat-area width, not a fixed narrow width.
  - [x] H1.1. Topics floating list should show each topic's code, name AND summary, formatted for readability.
...
- [x] I2. TPC2 - Build the GUI glossary/mapping skill: re-runnable skill producing a Markdown glossary (areas, features, elements; each with a stable ID plus short handle) and an interactive HTML map with clickable hotspot overlays over seeded multi-state screenshots. Artifacts under /features-gui-mapping. Output excludes code/css/js.
...
- [x] I4. [planner, opus/xhigh] Author the fix SPEC (writes a spec doc only, no source edits): complete durable-element inventory per AREA, each with a PERMANENT stable slug+name and how-identified (control-auto / meaningful id / data-gui marker); the data-gui marker schema; the new probe.js discovery+handle+signature-dedup algorithm (handle NEVER from free content text); the determinism plan; the docs-reconcile plan. MUST preserve already-clean existing handles so cross-references stay stable.
- [x] I5. [probe agent] Owns .claude/skills/gui-map/{probe.js,build.js}. Rewrite discovery: candidate = interactive controls + [data-gui] (+ meaningful id); DROP the classed-leaf-by-text branch; derive handle/name from data-gui slug / id / control label (never arbitrary content text); add structural-signature dedup (repeated siblings -> one representative). Keep test/gui-map.test.js green.
- [x] I6. [markup agent] Owns public/{index.html,app.js,gui.js}. Add inert data-gui (+ data-gui-name where needed) markers to the durable non-control elements per the planner inventory, using the PERMANENT slugs. No behavior change.
- [x] I7. [fixture agent] Owns .claude/skills/gui-map/{fixture-data.js,fixture-server.js}. Determinism: neutralize the ~1500ms meta pulse during capture and freeze clock/relative-time so the captured handle SET is reproducible run-to-run (two identical runs -> identical handles).
- [x] I8. [docs agent] Owns docs/README.md (GUI- handle convention - stability now real via markers), SKILL.md (reframe: auto-discovered from author-marked durable elements + controls; data excluded; new durable element needs a marker), docs/gui-map.md header note.
- [x] I9. [main/me] Regenerate the map (live fixture + Playwright); VERIFY durable-only + stable handles (run twice, diff -> identical) + no content; run npm test; commit per the Docs gate.
