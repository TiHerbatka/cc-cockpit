...
- The client turns the derived mode into a render detail level (see FEAT-display-mode): `focus` hides intermediate prose + thinking and collapses each turn's tools into one per-turn summary (matching the terminal), `verbose` expands everything, `normal` is the default; the client also offers a fourth, cockpit-only `focus+` (focus's tool handling but keeps Claude's prose) via manual override.
- Rendering pipeline: filter the item list per mode (drop hidden prose/thinking), then group tools. `focus`/`focus+` aggregate a whole turn's tools into **one per-turn summary** group (so `focus+` shows one quiet tool line per turn, not one group per consecutive run — which read almost like `normal`); `normal`/`verbose` group only consecutive runs by a threshold. Per-card open state resets on a mode switch so a mode shows its own default folding.
...
- The session-id used as that rename key (`ccSessionId`) is **reconciled** to the SDK's real session id when the `system/init` message arrives. A fresh session is created under the cockpit's own placeholder id, but the SDK writes its transcript under its own id (the one the Resume picker later passes back), so without this reconcile a fresh-session rename would be saved under an id that never recurs and would never restore on resume (E1 audit fix). A resume already starts on the real id, so its reconcile is a no-op.
...
**Key facts:**
- Safety is escape-first: the source text is fully HTML-escaped before any markup is constructed, and the only HTML emitted comes from the renderer's own templates with escaped interpolations — so no assistant text can inject live markup. Link hrefs are scheme-checked (only `http(s)`, `mailto`, `tel`, anchor, and relative URLs are allowed; `javascript:`/`data:`/other schemes are dropped to plain text). ASCII control characters are stripped from the URL **before** the scheme check, so a smuggled control byte (e.g. a leading SOH or an embedded tab in `java<TAB>script:`) can't slip a blocked scheme past the allowlist; protocol-relative `//host` URLs are also rejected.
- Inline code spans are escaped and held behind runtime sentinels before emphasis is applied, so markers inside backticks aren't reformatted **and** emphasis can still span across a code span (e.g. ``**bold with `code` inside**`` renders fully bold — a plain split would leave the `**` markers in different segments and emit them literally); `_` emphasis ignores `snake_case`.
- A block parser handles fenced code blocks (literal, escaped; an unclosed fence still renders, which keeps mid-stream output stable), headings, ordered/unordered lists (with multi-level nesting: indentation depth drives a stack of open list elements so child lists land inside the parent `<li>` before it closes), blockquotes, and horizontal rules; within a paragraph single newlines become `<br>`. Lines are split tolerant of CRLF (a trailing `\r` is dropped) so Windows line endings don't defeat the line-anchored block patterns.
- GitHub-flavored pipe tables: a header row followed by a delimiter row (cells of `---`, optionally with `:` for alignment — `:---` left, `---:` right, `:---:` center) renders as `<table class="md-table">` with `<thead>`/`<tbody>`; cell contents go through inline rendering (bold, code, links). A `|` line not followed by a valid delimiter row falls through to normal paragraph rendering (no misfires on a lone `|`).
