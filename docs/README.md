...
> **GUI map pending (A10).** `docs/gui-map.md` does not exist yet — the GUI glossary + visual map currently live under `features-gui-mapping/` (`glossary.md`, `map.html`), generated by the `/gui-map` skill. Wiring them in as `docs/gui-map.md`, and relocating the skill's mechanism files into `.claude/skills/gui-map/`, is TODO A10.
