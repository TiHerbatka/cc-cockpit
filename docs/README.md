...
- **Generated docs are exempt from per-entry stamping.** `docs/gui-map.md` + the visual map under `docs/gui-map/` are **generated — do not hand-edit and do not hand-stamp.** The glossary carries a single file-level **`Last generated: YYYY-MM-DD`** instead of per-entry dates, and its handles/names are derived mechanically from the live DOM. Its upkeep is **re-running the `/gui-map` skill** after any GUI change — that, not a date bump, brings it current. `GUI-` handles remain stable and referenceable.
