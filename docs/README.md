...
- `GUI-` handles are element-level (areas: SIDEBAR, HEADER, CONV, COMPOSE, PANEL, INTERACTION, MODAL, MENU) and complement the capability-level `FEAT-` handles — they say *where an element is and what it's called*, not *what a capability does*. Their stability is **enforced, not aspirational**: a `GUI-` handle's identity comes from an inert `data-gui` marker placed on the durable element in the product markup (or, for a plain control, its fixed accessible label) — never from rendered content — so it does not churn when fixture data, the clock, or a status value changes. The `<slug>` tail is the marker's permanent slug, chosen once and retired-not-renamed like every other handle.
...
- **Generated docs are exempt from per-entry stamping.** `docs/gui-map.md` + the visual map under `docs/gui-map/` are **generated — do not hand-edit and do not hand-stamp.** The glossary carries a single file-level **`Last generated: YYYY-MM-DD`** instead of per-entry dates. Each entry's identity is anchored to an inert `data-gui` marker in the product markup (or a stable interactive-control label), **not** to rendered content — so handles stay stable across data/clock/transition changes (genuinely enforced, not aspirational). **To add a new durable element to the map, place a `data-gui="<permanent-slug>"` marker on it in `public/` (optionally a `data-gui-name`), then re-run `/gui-map`** — a new durable element means a new marker; pure data is intentionally not mapped. Upkeep after any GUI change is **re-running the `/gui-map` skill** — that, not a date bump, brings it current. `GUI-` handles remain stable and referenceable.
